...
module fv_control_mod
  ...
  use fv_arrays_mod,       only: fv_atmos_type, allocate_fv_atmos_type, &
                                 deallocate_fv_atmos_type, R_GRID
...
!!! CLEANUP: should be replaced by a getter function?
  ...
  use mpp_mod,             only: mpp_send, mpp_sync, mpp_transmit, mpp_set_current_pelist, &
                                 mpp_declare_pelist, mpp_root_pe, mpp_recv, mpp_sync_self, &
                                 mpp_broadcast, read_input_nml
  use fv_diagnostics_mod,  only: fv_diag_init_gn

#ifdef MULTI_GASES
  use constants_mod,       only: rvgas, cp_air
  use multi_gases_mod,     only: multi_gases_init, rilist => ri, cpilist => cpi
#endif
...
contains
  ...
  subroutine fv_init(Atm, dt_atmos, grids_on_this_pe, p_split)
    ...
    ! This call is needed to set up the pointers for fv_current_grid,
    ! even for a single-grid run
    ...
    ntilesMe = size(Atm(:)) ! Full number of Atm arrays; one less than number of grids,
                            ! if multiple grids
    ...
    do n=1,ntilesMe
       ...
       if (grids_on_this_pe(n)) then
          ...
          if (grids_on_this_pe(n)) then
             ...
             if ( (Atm(n)%bd%iec-Atm(n)%bd%isc+1).lt.4 .or. (Atm(n)%bd%jec-Atm(n)%bd%jsc+1).lt.4 ) then
                ...
                call mpp_error(FATAL,'Domain Decomposition:  Cubed Sphere compute domain has a &
                     &minimum requirement of 4 points in X and Y, respectively')
             end if
          ...
          ! Need to SEND grid_global to any child grids;
          ! this is received in setup_aligned_nest in fv_grid_tools
          if (Atm(n)%neststruct%nested) then

             call mpp_get_global_domain(Atm(n)%parent_grid%domain, &
                  isg, ieg, jsg, jeg)

             ! FIXME: Should replace this by generating the global grid
             !        (or at least one face thereof) on the
             !        nested PEs instead of sending it around.
             if (gid == Atm(n)%parent_grid%pelist(1)) then
                call mpp_send( &
                     Atm(n)%parent_grid%grid_global(isg-ng:ieg+1+ng,jsg-ng:jeg+1+ng,1:2,parent_tile), &
             ...
             if (Atm(n)%neststruct%twowaynest) then
                ...
                endif ! Atm(n)%parent_grid%tile == Atm(n)%neststruct%parent_tile

             endif ! Atm(n)%neststruct%twowaynest

          endif ! Atm(n)%neststruct%nested
       endif ! grids_on_this_pe(n)
    enddo ! n=1,ntilesMe
  ...
  subroutine run_setup(Atm, dt_atmos, grids_on_this_pe, p_split)
    ...
    namelist /fv_core_nml/npx, npy, ntiles, npz, npz_rst, &
        ...
        print_freq, write_3d_diags, do_schmidt, &
        ...
        external_ic, read_increment, ncep_ic, nggps_ic, ecmwf_ic, &
        use_new_ncep, use_ncep_phy, fv_diag_ic, external_eta, &
        res_latlon_dynamics, res_latlon_tracers, &
        scale_z, w_max, z_min, lim_fac, &
        dddmp, d2_bg, d4_bg, vtdm4, trdm2, d_ext, delt_max, &
        beta, non_ortho, n_sponge, &
        warm_start, adjust_dry_mass, mountain, &
        d_con, ke_bg, nord, nord_tr, convert_ke, use_old_omega, &
        ...
        range_warn, dwind_2d, inline_q, z_tracer, reproduce_sum, &
        adiabatic, do_vort_damp, no_dycore, &
        tau, tau_h2o, rf_cutoff, nf_omega, &
        hydrostatic, fv_sg_adj, breed_vortex_inline, &
        na_init, nudge_dz, hybrid_z, Make_NH, &
        n_zs_filter, nord_zs_filter, full_zs_filter, reset_eta, &
        pnats, dnats, a2b_ord, remap_t, p_ref, d2_bg_k1, d2_bg_k2, &
        c2l_ord, dx_const, dy_const, umax, deglat, &
        deglon_start, deglon_stop, deglat_start, deglat_stop, &
        phys_hydrostatic, use_hydro_pressure, make_hybrid_z, old_divg_damp, &
        add_noise, nested, twowaynest, parent_grid_num, parent_tile, nudge_qv, &
    ...
    do n=1,size(Atm)
       ...
       if (n > 1) then
          ...
          print'("[INFO] WDR fv_control.F90 run_setup about to read nested namelist file: ",A20)', nested_grid_filename
       ...
       if (Atm(n)%neststruct%nested) then
          do i=1,n-1
             if (Atm(i)%grid_number == parent_grid_num) then
                print'("[INFO] WDR fv_control.F90 run_setup i ",I0," n=",I0," parent_grid_num=",I0," parent_grid_tile=",I0)', i, n, parent_grid_num, parent_tile
          ...
          ! Note that if a gnomonic grid has a parent
          ! it is a NESTED gnomonic grid and therefore only has one tile
          if ( Atm(n)%parent_grid%flagstruct%grid_type < 3 .and. &
             ...
             if (parent_tile > 6 .or. parent_tile < 1) then
                ! Note: This may change for telescoping nests,
                !       when the parent_tile may refer to another
                !       inner nest whose tile number > 6. (Ahern)
       ...
       else ! .not. (Atm(n)%neststruct%nested)
       ...
       ! Q: Could this just be put in the if-block immediately above,
       !    which has the same condition? (Ahern)
       if (Atm(n)%neststruct%nested) then
...
199    format(A,i3.3)
    ...
    ! CLEANUP: This call sets the pelist to ALL, which is also
  ...
  subroutine init_nesting(Atm, grids_on_this_pe, p_split)
    ...
    print '("[INFO] WDR init_nesting about to allocate Atm(ngrids) ngrids=",I0)', ngrids
    ...
    if (ngrids == 1) then
       ...
       print '("[INFO] WDR init_nesting iterating over ngrids: ",I0)', ngrids

       do n=1,ngrids
          print '("[INFO] WDR init_nesting iterating over grid n ",I0," nest_pes(n) ",I0)', n, nest_pes(n)
          if (n == 1) then
             pe_list_name = ''
          else
          ...
          if (nest_pes(n) == 0) then
             print '("[INFO] WDR inside nest_pes(n)=0 for grid n ", I0)', n
             ...
             if (n > 1) then
                ...
                if(is_master()) then
                   write(*,*) '[INFO] WDR init_nesting attempting to read nested namelist file: ', 'input_'//trim(pe_list_name)//'.nml'
                   print*, ''
                endif
             ...
             print  '("[INFO] WDR added nest_pes ",I0," to pecounter ",I0," for nest n ",I0)', nest_pes(n),  pecounter, n
          endif
       enddo ! n=1,ngrids
  ...
  subroutine setup_pointers(Atm)
    ...
    ! This routine associates the MODULE flag pointers with
    ! the ARRAY flag variables for the grid active on THIS pe
    ! so the flags can be read in from the namelist.
