...
! Read Main namelist
   ...
   ! Set input_file_nml for correct parent/nest initialization
      if (n > 1) then
         ...
         ! Bill Ramstrom - Report attempt to read namelist for nest
         ! print'("[INFO] fv_control.F90 run_setup about to read nested namelist file: ",A20)', nested_grid_filename
      ...
      if (Atm(n)%neststruct%nested) then
         do i=1,n-1
            if (Atm(i)%grid_number == parent_grid_num) then
               ! print'("[INFO] fv_control.F90 run_setup i ",I0," n=",I0," parent_grid_num=",I0," parent_grid_tile=",I0)', i, n, parent_grid_num, parent_tile
         ...
         do n=1,ngrids
            ...
            if (nest_pes(n) == 0) then
               ...
               if (n > 1) then
                  ...
                  if(is_master()) then
                    ! Bill Ramstrom - Report attempt to read nest namelist file
                    write(*,*) '[INFO] init_nesting attempting to read nested namelist file: ', 'input_'//trim(pe_list_name)//'.nml'
                    print*, ''
                  endif
