...
!--------------------
!
      integer :: i,i_start,i_end,j,j_start,j_end,klev_out,sec
!
      real :: ps1
!
!-----------------------------------------------------------------------
...
!xxx  regional_bc_bounds%je_north=0
...
!
!xxx  regional_bc_bounds%js_south=jed-nhalo_model+1
...
!xxx  regional_bc_bounds%ie_east=0
...
!
      if (west_bc) then
!xxx  regional_bc_bounds%is_west=ied-nhalo_model+1
...
!xxx  regional_bc_bounds%je_north_uvs=1
...
!xxx  regional_bc_bounds%je_north_uvw=0
...
!
!xxx  regional_bc_bounds%js_south_uvs=jed-nhalo_model+1
...
!
!xxx  regional_bc_bounds%js_south_uvw=jed-nhalo_model+1
...
!xxx  regional_bc_bounds%ie_east_uvs=0
...
!xxx  regional_bc_bounds%ie_east_uvw=0   !<-- east side of cell at i=0
...
!
      if (west_bc) then
!xxx  regional_bc_bounds%is_west_uvs=ied-nhalo_model+1
...
!     if(nside==3.and.mpp_pe()==20)then
!       write(0,44611)bc_side_t1%u_BC(1,164:166,64)
44611   format(' after remap_dwinds u_BC(1,164:166,64)=',3(1x,e12.5))
!     endif
...
!xxxxxxxxx    j_end_array=js_input+nhalo_data
              j_end_array=js_input+nhalo_data+nrows_blend
            else
!xxxxxxxx     j_end_array=js_input+nhalo_data-1
...
!
            i_start_array=is_input
            i_end_array  =ie_input
!xxxxxxxxx  j_start_array=je_input-nhalo_data+1
...
!xxxxxxx      i_end_array=is_input+nhalo_data
              i_end_array=is_input+nhalo_data+nrows_blend
            else
!xxxxxxx      i_end_array=is_input+nhalo_data-1
...
!
            ...
            j_end_array  =je_input
!
!xxxxxxx    i_start_array=ie_input-nhalo_data+1
...
!
      is=is_bc
      if(side=='west')then
!xxx    is=ie_bc-nhalo_data+1
        is=ie_bc-nhalo_data-nrows_blend+1
      endif
!
      ie=ie_bc
      if(side=='east')then
!xxx    ie=is_bc+nhalo_data-1
        ie=is_bc+nhalo_data+nrows_blend-1
      endif
!
      js=js_bc
      if(side=='south')then
!xxx    js=je_bc-nhalo_data+1
        js=je_bc-nhalo_data-nrows_blend+1
      endif
!
      je=je_bc
      if(side=='north')then
!xxx    je=js_bc+nhalo_data-1
...
!     if(i==210.and.j==1.and.k==1)then
!       write(0,77451)BC_side%vc_BC(i,j,k),vc(i,j,k)
77451   format(' remap_dwinds BC_side%vc_BC=',e12.5,' vc=',e12.5)
!     endif
        enddo
     enddo
...
!
            ...
            j1_blend=j2_blend-nrows_blend+1
            i_bc=-9e9
            j_bc=j1
!
          endif
...
!
            i1_blend=is
            i2_blend=is+nrows_blend-1
            j1_blend=js
            j2_blend=je
            if(north_bc)then
              j1_blend=js+nrows_blend
            endif
            if(south_bc)then
              j2_blend=je-nrows_blend
            endif
            if(trim(bc_vbl_name)=='u'.or.trim(bc_vbl_name)=='vc')then
              j2_blend=j2_blend+1
            endif
...
!
      ...
      enddo
!
!  if(trim(bc_vbl_name)/='delp')then
!  if(trim(bc_vbl_name)/='pt')then
!  if(trim(bc_vbl_name)/='delz')then
!  if(trim(bc_vbl_name)/='vc')then
!  if(trim(bc_vbl_name)/='uc')then
!  if(trim(bc_vbl_name)/='divgd')then
!  if(trim(bc_vbl_name)/='q')then
!  if(trim(bc_vbl_name)/='q_con')then
!  if(trim(bc_vbl_name)/='w')then
!  if(trim(bc_vbl_name)/='u')then
!  if(trim(bc_vbl_name)/='v')then
!    write(0,*)' returning from bc_time_interpolation'
!    return
!  endif
!---------------------------------------------------------------------
